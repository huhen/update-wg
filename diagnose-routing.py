...
def check_wireguard():
    ...
    try:
        ...
        # Проверяем AllowedIPs в Peer секции
        if 'AllowedIPs = 0.0.0/1, 128.0.0.0/1' in content or 'AllowedIPs = 0.0.0.0/1, 128.0.0/1' in content:
            print("✅ Параметр 'AllowedIPs = 0.0.0.0/1, 128.0.0.0/1' установлен в конфигурации wg1")
        else:
            print("❌ Параметр 'AllowedIPs = 0.0.0.0/1, 128.0.0.0/1' НЕ установлен в конфигурации wg1")
            # Проверим, какие AllowedIPs указаны
            import re
            allowed_ips_match = re.search(r'AllowedIPs\s*=\s*(.+)', content)
            if allowed_ips_match:
                print(f"ℹ️  Вместо этого найдены AllowedIPs: {allowed_ips_match.group(1)}")
    except Exception as e:
...
def check_default_route():
    """Проверяет маршрут по умолчанию в основной таблице"""
    print(f"\n🔍 Проверка маршрута по умолчанию...")
    
    result = execute_command("ip route show", "Проверка маршрута по умолчанию в основной таблице")
    if result:
        default_routes = [line for line in result.split('\n') if 'default' in line]
        if default_routes:
            for route in default_routes:
                print(f"   Маршрут по умолчанию: {route.strip()}")
                if 'wg1' in route:
                    print(f"❌ Маршрут по умолчанию идет через wg1 (это может быть нежелательно)")
                elif 'ens3' in route:
                    print(f"✅ Маршрут по умолчанию идет через ens3 (это правильно)")
                else:
                    print(f"ℹ️  Маршрут по умолчанию идет через другой интерфейс: {route.strip()}")
        else:
            print(f"❌ Не найден маршрут по умолчанию в основной таблице")
    else:
        print(f"❌ Не удалось получить маршрут по умолчанию")
...
def main():
    ...
    print("8. Убедитесь, что маршрут по умолчанию в основной таблице ведет через ens3, а не через wg1")
